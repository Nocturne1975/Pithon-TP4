...
class SimpleParser(ast.NodeVisitor):
    ...
    def parse(self, source_code: str):
        try:
            ...
            raise SyntaxError(f"Erreur de syntaxe ligne {e.lineno}: {e.msg}")
        except Exception as e:
            raise ValueError(f"Erreur lors de l'analyse du code: {e}")
    ...
    def generic_visit(self, node):
        raise ValueError(f"Construction  '{type(node).__name__}' non supportée dans pithon."
                         f"Consultez la documentation pour les fonctionnalites disponibles.")
